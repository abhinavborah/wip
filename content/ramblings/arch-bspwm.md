...
Shadows, fading, inactive opacity so unfocused windows step back. Nothing flashy, just enough to feel polished. VSync enabled, xrender backend for broad hardware support.

## dunst (notification manager)

Notifications in the top-right corner with the yellow frame. Normal ones timeout after 10 seconds, critical ones stick around until you deal with them. Blends in with the rest of the theme.

## what I'd change

Always more to tweak. An install script would be nice, network modules in polybar would be handy, theme switching for variety. The scripts directory I mention isn't in the repo. Wayland isn't happening since bspwm is X11 only, which is fine by me.
